var dropdown = {
    propertyType: [{ name: "Commercial" }, { name: "Residential" }],
    propertySubcategory: [
        { type: "Commercial", name: "Commercial Shops" },
        { type: "Commercial", name: "Commercial Showrooms" },
        { type: "Commercial", name: "Commercial Office/Space" },
        { type: "Commercial", name: "Commercial Land/Building" },
        { type: "Commercial", name: "Industrial Lands/Plots" },
        { type: "Commercial", name: "Agricultural Land" },
        { type: "Commercial", name: "Hotel/Resorts" },
        { type: "Commercial", name: "Guest-House/Banquet-Halls" },
        { type: "Commercial", name: "Institutional Land/Building" },
        { type: "Commercial", name: "Other" },
        { type: "Residential", name: "Apartment" },
        { type: "Residential", name: "Independent/Builder Floor" },
        { type: "Residential", name: "Independent House/Villa" },
        { type: "Residential", name: "Residential Land" },
        { type: "Residential", name: "Studio Apartment" },
        { type: "Residential", name: "Farm House" },
        { type: "Residential", name: "Serviced Apartments" },
        { type: "Residential", name: "Other" },
    ],
    leadType: [{ name: "Tenant" }, { name: "Buyer" }, { name: "Investor" }],
    propertyCity: [
        { name: "Noida" },
        { name: "Greater Noida" },
        { name: "Gr. Noida West" },
        { name: "Delhi" }, { name: "YEIDA City" },
        { name: "Gurugram" },
        { name: "Faridabad" },
        { name: "Ghaziabad" },
        { name: "Lucknow" },
        { name: "Mumbai" },
        { name: "Delhi" },
        { name: "Bengaluru" },
        { name: "Ahmedabad" },
        { name: "Hyderabad" },
        { name: "Chennai" },
        { name: "Kolkata" },
        { name: "Pune" },
        { name: "Jaipur" },
        { name: "Surat" },
        { name: "Lucknow" },
        { name: "Kanpur" },
        { name: "Nagpur" },
        { name: "Patna" },
        { name: "Indore" },
        { name: "Thane" },
        { name: "Bhopal" },
        { name: "Visakhapatnam" },
        { name: "Vadodara" },
        { name: "Firozabad" },
        { name: "Ludhiana" },
        { name: "Rajkot" },
        { name: "Agra" },
        { name: "Siliguri" },
        { name: "Nashik" },
        { name: "Faridabad" },
        { name: "Patiala" },
        { name: "Meerut" },
        { name: "Kalyan-Dombivali" },
        { name: "Vasai-Virar" },
        { name: "Varanasi" },
        { name: "Srinagar/Jammu and Kashmir" },
        { name: "Dhanbad" },
        { name: "Jodhpur" },
        { name: "Amritsar" },
        { name: "Raipur" },
        { name: "Allahabad" },
        { name: "Coimbatore" },
        { name: "Jabalpur" },
        { name: "Gwalior" },
        { name: "Vijayawada" },
        { name: "Madurai" },
        { name: "Guwahati" },
        { name: "Chandigarh" },
        { name: "Hubli-Dharwad" },
        { name: "Amroha" },
        { name: "Moradabad" },
        { name: "Gurgaon" },
        { name: "Aligarh" },
        { name: "Solapur" },
        { name: "Ranchi" },
        { name: "Jalandhar" },
        { name: "Tiruchirappalli" },
        { name: "Bhubaneswar" },
        { name: "Salem" },
        { name: "Warangal" },
        { name: "Mira-Bhayandar" },
        { name: "Thiruvananthapuram" },
        { name: "Bhiwandi" },
        { name: "Saharanpur" },
        { name: "Guntur" },
        { name: "Amravati" },
        { name: "Bikaner" },
        { name: "Noida" },
        { name: "Jamshedpur" },
        { name: "Bhilai Nagar" },
        { name: "Cuttack" },
        { name: "Kochi" },
        { name: "Udaipur/Rajasthan" },
        { name: "Bhavnagar" },
        { name: "Dehradun" },
        { name: "Asansol" },
        { name: "Nanded-Waghala" },
        { name: "Ajmer" },
        { name: "Jamnagar" },
        { name: "Ujjain" },
        { name: "Sangli" },
        { name: "Loni" },
        { name: "Jhansi" },
        { name: "Pondicherry" },
        { name: "Nellore" },
        { name: "Jammu" },
        { name: "Belagavi" },
        { name: "Raurkela" },
        { name: "Mangaluru" },
        { name: "Tirunelveli" },
        { name: "Malegaon" },
        { name: "Gaya" },
        { name: "Tiruppur" },
        { name: "Davanagere" },
        { name: "Kozhikode" },
        { name: "Akola" },
        { name: "Kurnool" },
        { name: "Bokaro Steel City" },
        { name: "Rajahmundry" },
        { name: "Ballari" },
        { name: "Agartala" },
        { name: "Bhagalpur" },
        { name: "Latur" },
        { name: "Dhule" },
        { name: "Korba" },
        { name: "Bhilwara" },
        { name: "Brahmapur" },
        { name: "Mysore" },
        { name: "Muzaffarpur" },
        { name: "Ahmednagar" },
        { name: "Kollam" },
        { name: "Raghunathganj" },
        { name: "Bilaspur" },
        { name: "Shahjahanpur" },
        { name: "Thrissur" },
        { name: "Alwar" },
        { name: "Kakinada" },
        { name: "Nizamabad" },
        { name: "Sagar" },
        { name: "Tumkur" },
        { name: "Hisar" },
        { name: "Rohtak" },
        { name: "Panipat" },
        { name: "Darbhanga" },
        { name: "Kharagpur" },
        { name: "Aizawl" },
        { name: "Ichalkaranji" },
        { name: "Tirupati" },
        { name: "Karnal" },
        { name: "Bathinda" },
        { name: "Rampur" },
        { name: "Shivamogga" },
        { name: "Ratlam" },
        { name: "Modinagar" },
        { name: "Durg" },
        { name: "Shillong" },
        { name: "Imphal" },
        { name: "Hapur" },
        { name: "Ranipet" },
        { name: "Anantapur" },
        { name: "Arrah" },
        { name: "Karimnagar" },
        { name: "Parbhani" },
        { name: "Etawah" },
        { name: "Bharatpur" },
        { name: "Begusarai" },
        { name: "New Delhi" },
        { name: "Chhapra/Bihar" },
        { name: "Kadapa" },
        { name: "Ramagundam" },
        { name: "Pali/Rajasthan" },
        { name: "Satna" },
        { name: "Vizianagaram" },
        { name: "Katihar" },
        { name: "Hardwar" },
        { name: "Sonipat" },
        { name: "Nagercoil" },
        { name: "Thanjavur" },
        { name: "Murwara (Katni)" },
        { name: "Naihati" },
        { name: "Sambhal" },
        { name: "Nadiad" },
        { name: "Yamunanagar" },
        { name: "English Bazar" },
        { name: "Eluru" },
        { name: "Munger" },
        { name: "Panchkula" },
        { name: "Raayachuru" },
        { name: "Panvel" },
        { name: "Deoghar" },
        { name: "Ongole" },
        { name: "Nandyal" },
        { name: "Morena" },
        { name: "Bhiwani" },
        { name: "Porbandar" },
        { name: "Palakkad" },
        { name: "Anand" },
        { name: "Purnia" },
        { name: "Baharampur" },
        { name: "Barmer" },
        { name: "Morvi" },
        { name: "Orai" },
        { name: "Bahraich" },
        { name: "Sikar" },
        { name: "Vellore" },
        { name: "Singrauli" },
        { name: "Khammam" },
        { name: "Mahesana" },
        { name: "Silchar" },
        { name: "Sambalpur" },
        { name: "Rewa" },
        { name: "Unnao" },
        { name: "Hugli-Chinsurah" },
        { name: "Raiganj" },
        { name: "Phusro" },
        { name: "Adityapur" },
        { name: "Alappuzha" },
        { name: "Bahadurgarh" },
        { name: "Machilipatnam" },
        { name: "Rae Bareli" },
        { name: "Jalpaiguri" },
        { name: "Bharuch" },
        { name: "Pathankot" },
        { name: "Hoshiarpur" },
        { name: "Baramula" },
        { name: "Adoni" },
        { name: "Jind" },
        { name: "Tonk" },
        { name: "Tenali" },
        { name: "Kancheepuram" },
        { name: "Vapi/Gujarat" },
        { name: "Sirsa" },
        { name: "Navsari" },
        { name: "Mahbubnagar" },
        { name: "Puri" },
        { name: "Robertson Pet" },
        { name: "Erode" },
        { name: "Batala" },
        { name: "Haldwani-cum-Kathgodam" },
        { name: "Vidisha" },
        { name: "Saharsa" },
        { name: "Thanesar" },
        { name: "Chittoor" },
        { name: "Veraval" },
        { name: "Lakhimpur" },
        { name: "Sitapur" },
        { name: "Hindupur" },
        { name: "Santipur" },
        { name: "Balurghat" },
        { name: "Ganjbasoda" },
        { name: "Moga" },
        { name: "Proddatur" },
        { name: "Srinagar" },
        { name: "Medinipur" },
        { name: "Habra" },
        { name: "Sasaram" },
        { name: "Hajipur" },
        { name: "Bhuj" },
        { name: "Shivpuri" },
        { name: "Ranaghat" },
        { name: "Shimla" },
        { name: "Tiruvannamalai" },
        { name: "Kaithal" },
        { name: "Rajnandgaon" },
        { name: "Godhra" },
        { name: "Hazaribag" },
        { name: "Bhimavaram" },
        { name: "Mandsaur" },
        { name: "Dibrugarh" },
        { name: "Kolar" },
        { name: "Bankura" },
        { name: "Mandya" },
        { name: "Dehri-on-Sone" },
        { name: "Madanapalle" },
        { name: "Malerkotla" },
        { name: "Lalitpur" },
        { name: "Bettiah" },
        { name: "Pollachi" },
        { name: "Khanna" },
        { name: "Neemuch" },
        { name: "Palwal" },
        { name: "Palanpur" },
        { name: "Guntakal" },
        { name: "Nabadwip" },
        { name: "Udupi" },
        { name: "Jagdalpur" },
        { name: "Motihari" },
        { name: "Pilibhit" },
        { name: "Dimapur" },
        { name: "Mohali" },
        { name: "Sadulpur" },
        { name: "Rajapalayam" },
        { name: "Dharmavaram" },
        { name: "Kashipur" },
        { name: "Sivakasi" },
        { name: "Darjiling" },
        { name: "Chikkamagaluru" },
        { name: "Gudivada" },
        { name: "Baleshwar Town" },
        { name: "Mancherial" },
        { name: "Srikakulam" },
        { name: "Adilabad" },
        { name: "Yavatmal" },
        { name: "Barnala" },
        { name: "Nagaon" },
        { name: "Narasaraopet" },
        { name: "Raigarh" },
        { name: "Roorkee" },
        { name: "Valsad" },
        { name: "Ambikapur" },
        { name: "Giridih" },
        { name: "Chandausi" },
        { name: "Purulia" },
        { name: "Patan" },
        { name: "Bagaha" },
        { name: "Hardoi " },
        { name: "Achalpur" },
        { name: "Osmanabad" },
        { name: "Deesa" },
        { name: "Nandurbar" },
        { name: "Azamgarh" },
        { name: "Ramgarh" },
        { name: "Firozpur" },
        { name: "Baripada Town" },
        { name: "Karwar" },
        { name: "Siwan" },
        { name: "Rajampet" },
        { name: "Pudukkottai" },
        { name: "Anantnag" },
        { name: "Tadpatri" },
        { name: "Satara" },
        { name: "Bhadrak" },
        { name: "Kishanganj" },
        { name: "Suryapet" },
        { name: "Wardha" },
        { name: "Ranebennuru" },
        { name: "Amreli" },
        { name: "Neyveli (TS)" },
        { name: "Jamalpur" },
        { name: "Marmagao" },
        { name: "Udgir" },
        { name: "Tadepalligudem" },
        { name: "Nagapattinam" },
        { name: "Buxar" },
        { name: "Aurangabad/Maharashtra" },
        { name: "Jehanabad" },
        { name: "Phagwara" },
        { name: "Khair" },
        { name: "Sawai Madhopur" },
        { name: "Kapurthala" },
        { name: "Chilakaluripet" },
        { name: "Aurangabad/Bihar" },
        { name: "Malappuram" },
        { name: "Rewari" },
        { name: "Nagaur" },
        { name: "Sultanpur" },
        { name: "Nagda" },
        { name: "Port Blair" },
        { name: "Lakhisarai" },
        { name: "Panaji" },
        { name: "Tinsukia" },
        { name: "Itarsi" },
        { name: "Kohima" },
        { name: "Balangir" },
        { name: "Nawada" },
        { name: "Jharsuguda" },
        { name: "Jagtial" },
        { name: "Viluppuram" },
        { name: "Amalner" },
        { name: "Zirakpur" },
        { name: "Tanda" },
        { name: "Tiruchengode" },
        { name: "Nagina" },
        { name: "Yemmiganur" },
        { name: "Vaniyambadi" },
        { name: "Sarni" },
        { name: "Theni Allinagaram" },
        { name: "Margao" },
        { name: "Akot" },
        { name: "Sehore" },
        { name: "Mhow Cantonment" },
        { name: "Kot Kapura" },
        { name: "Makrana" },
        { name: "Pandharpur" },
        { name: "Miryalaguda" },
        { name: "Shamli" },
        { name: "Seoni" },
        { name: "Ranibennur" },
        { name: "Kadiri" },
        { name: "Shrirampur" },
        { name: "Rudrapur/Uttarakhand" },
        { name: "Parli" },
        { name: "Najibabad" },
        { name: "Nirmal" },
        { name: "Udhagamandalam" },
        { name: "Shikohabad" },
        { name: "Jhumri Tilaiya" },
        { name: "Aruppukkottai" },
        { name: "Ponnani" },
        { name: "Jamui" },
        { name: "Sitamarhi" },
        { name: "Chirala" },
        { name: "Anjar" },
        { name: "Karaikal" },
        { name: "Hansi" },
        { name: "Anakapalle" },
        { name: "Mahasamund" },
        { name: "Faridkot" },
        { name: "Saunda" },
        { name: "Dhoraji" },
        { name: "Paramakudi" },
        { name: "Balaghat" },
        { name: "Sujangarh" },
        { name: "Khambhat" },
        { name: "Muktsar" },
        { name: "Rajpura" },
        { name: "Kavali" },
        { name: "Dhamtari" },
        { name: "Ashok Nagar" },
        { name: "Sardarshahar" },
        { name: "Mahuva" },
        { name: "Bargarh" },
        { name: "Kamareddy" },
        { name: "Sahibganj" },
        { name: "Kothagudem" },
        { name: "Ramanagaram" },
        { name: "Gokak" },
        { name: "Tikamgarh" },
        { name: "Araria" },
        { name: "Rishikesh" },
        { name: "Shahdol" },
        { name: "Medininagar (Daltonganj)" },
        { name: "Arakkonam" },
        { name: "Washim" },
        { name: "Sangrur" },
        { name: "Bodhan" },
        { name: "Fazilka" },
        { name: "Palacole" },
        { name: "Keshod" },
        { name: "Sullurpeta" },
        { name: "Wadhwan" },
        { name: "Gurdaspur" },
        { name: "Vatakara" },
        { name: "Tura" },
        { name: "Narnaul" },
        { name: "Kharar" },
        { name: "Yadgir" },
        { name: "Ambejogai" },
        { name: "Ankleshwar" },
        { name: "Savarkundla" },
        { name: "Paradip" },
        { name: "Virudhachalam" },
        { name: "Kanhangad" },
        { name: "Kadi" },
        { name: "Srivilliputhur" },
        { name: "Gobindgarh" },
        { name: "Tindivanam" },
        { name: "Mansa/Punjab" },
        { name: "Taliparamba" },
        { name: "Manmad" },
        { name: "Tanuku" },
        { name: "Rayachoti" },
        { name: "Virudhunagar" },
        { name: "Koyilandy" },
        { name: "Jorhat" },
        { name: "Karur" },
        { name: "Valparai" },
        { name: "Srikalahasti" },
        { name: "Neyyattinkara" },
        { name: "Bapatla" },
        { name: "Fatehabad" },
        { name: "Malout" },
        { name: "Sankarankovil" },
        { name: "Tenkasi" },
        { name: "Ratnagiri" },
        { name: "Rabkavi Banhatti" },
        { name: "Sikandrabad" },
        { name: "Chaibasa" },
        { name: "Chirmiri" },
        { name: "Palwancha" },
        { name: "Bhawanipatna" },
        { name: "Kayamkulam" },
        { name: "Pithampur" },
        { name: "Nabha" },
        { name: "Shahabad, Hardoi" },
        { name: "Dhenkanal" },
        { name: "Uran Islampur" },
        { name: "Gopalganj" },
        { name: "Bongaigaon City" },
        { name: "Palani" },
        { name: "Pusad" },
        { name: "Sopore" },
        { name: "Pilkhuwa" },
        { name: "Tarn Taran" },
        { name: "Renukoot" },
        { name: "Mandamarri" },
        { name: "Shahabad" },
        { name: "Barbil" },
        { name: "Koratla" },
        { name: "Madhubani" },
        { name: "Arambagh" },
        { name: "Gohana" },
        { name: "Ladnu" },
        { name: "Pattukkottai" },
        { name: "Sirsi/Karnataka" },
        { name: "Sircilla" },
        { name: "Tamluk" },
        { name: "Jagraon" },
        { name: "AlipurdUrban Agglomerationr" },
        { name: "Alirajpur" },
        { name: "Tandur" },
        { name: "Naidupet" },
        { name: "Tirupathur/Tamil Nadu" },
        { name: "Tohana" },
        { name: "Ratangarh" },
        { name: "Dhubri" },
        { name: "Masaurhi" },
        { name: "Visnagar" },
        { name: "Vrindavan" },
        { name: "Nokha/Rajasthan" },
        { name: "Nagari" },
        { name: "Narwana" },
        { name: "Ramanathapuram" },
        { name: "Ujhani" },
        { name: "Samastipur" },
        { name: "Laharpur" },
        { name: "Sangamner" },
        { name: "Nimbahera" },
        { name: "Siddipet" },
        { name: "Suri" },
        { name: "Diphu" },
        { name: "Jhargram" },
        { name: "Shirpur-Warwade" },
        { name: "Tilhar" },
        { name: "Sindhnur" },
        { name: "Udumalaipettai" },
        { name: "Malkapur" },
        { name: "Wanaparthy" },
        { name: "Gudur" },
        { name: "Kendujhar" },
        { name: "Mandla" },
        { name: "Mandi" },
        { name: "Nedumangad" },
        { name: "North Lakhimpur" },
        { name: "Vinukonda" },
        { name: "Tiptur" },
        { name: "Gobichettipalayam" },
        { name: "Sunabeda" },
        { name: "Wani" },
        { name: "Upleta" },
        { name: "Narasapuram" },
        { name: "Nuzvid" },
        { name: "Tezpur" },
        { name: "Una" },
        { name: "Markapur" },
        { name: "Sheopur" },
        { name: "Thiruvarur" },
        { name: "Sidhpur" },
        { name: "Sahaswan" },
        { name: "Suratgarh" },
        { name: "Shajapur" },
        { name: "Rayagada" },
        { name: "Lonavla" },
        { name: "Ponnur" },
        { name: "Kagaznagar" },
        { name: "Gadwal" },
        { name: "Bhatapara" },
        { name: "Kandukur" },
        { name: "Sangareddy" },
        { name: "Unjha" },
        { name: "Lunglei" },
        { name: "Karimganj" },
        { name: "Kannur" },
        { name: "Bobbili" },
        { name: "Mokameh" },
        { name: "Talegaon Dabhade" },
        { name: "Anjangaon" },
        { name: "Mangrol/Gujarat" },
        { name: "Sunam" },
        { name: "Gangarampur" },
        { name: "Thiruvallur" },
        { name: "Tirur" },
        { name: "Rath" },
        { name: "Jatani" },
        { name: "Viramgam" },
        { name: "Rajsamand" },
        { name: "Yanam" },
        { name: "Kottayam" },
        { name: "Panruti" },
        { name: "Dhuri" },
        { name: "Namakkal" },
        { name: "Kasaragod" },
        { name: "Modasa" },
        { name: "Rayadurg" },
        { name: "Supaul" },
        { name: "Kunnamkulam" },
        { name: "Umred" },
        { name: "Bellampalle" },
        { name: "Sibsagar" },
        { name: "Mandi Dabwali" },
        { name: "Ottappalam" },
        { name: "Dumraon" },
        { name: "Samalkot" },
        { name: "Jaggaiahpet" },
        { name: "Goalpara" },
        { name: "Tuni" },
        { name: "Lachhmangarh" },
        { name: "Bhongir" },
        { name: "Amalapuram" },
        { name: "Firozpur Cantt." },
        { name: "Vikarabad" },
        { name: "Thiruvalla" },
        { name: "Sherkot" },
        { name: "Palghar" },
        { name: "Shegaon" },
        { name: "Jangaon" },
        { name: "Bheemunipatnam" },
        { name: "Panna" },
        { name: "Thodupuzha" },
        { name: "KathUrban Agglomeration" },
        { name: "Palitana" },
        { name: "Arwal" },
        { name: "Venkatagiri" },
        { name: "Kalpi" },
        { name: "Rajgarh (Churu)" },
        { name: "Sattenapalle" },
        { name: "Arsikere" },
        { name: "Ozar" },
        { name: "Thirumangalam" },
        { name: "Petlad" },
        { name: "Nasirabad" },
        { name: "Phaltan" },
        { name: "Rampurhat" },
        { name: "Nanjangud" },
        { name: "Forbesganj" },
        { name: "Tundla" },
        { name: "BhabUrban Agglomeration" },
        { name: "Sagara" },
        { name: "Pithapuram" },
        { name: "Sira" },
        { name: "Bhadrachalam" },
        { name: "Charkhi Dadri" },
        { name: "Chatra" },
        { name: "Palasa Kasibugga" },
        { name: "Nohar" },
        { name: "Yevla" },
        { name: "Sirhind Fatehgarh Sahib" },
        { name: "Bhainsa" },
        { name: "Parvathipuram" },
        { name: "Shahade" },
        { name: "Chalakudy" },
        { name: "Narkatiaganj" },
        { name: "Kapadvanj" },
        { name: "Macherla" },
        { name: "Raghogarh-Vijaypur" },
        { name: "Rupnagar" },
        { name: "Naugachhia" },
        { name: "Sendhwa" },
        { name: "Byasanagar" },
        { name: "Sandila" },
        { name: "Gooty" },
        { name: "Salur" },
        { name: "Nanpara" },
        { name: "Sardhana" },
        { name: "Vita" },
        { name: "Gumia" },
        { name: "Puttur/Karnataka" },
        { name: "Jalandhar Cantt." },
        { name: "Nehtaur" },
        { name: "Changanassery" },
        { name: "Mandapeta" },
        { name: "Dumka" },
        { name: "Seohara" },
        { name: "Umarkhed" },
        { name: "Madhupur" },
        { name: "Vikramasingapuram" },
        { name: "Punalur" },
        { name: "Kendrapara" },
        { name: "Sihor" },
        { name: "Nellikuppam" },
        { name: "Samana" },
        { name: "Warora" },
        { name: "Nilambur" },
        { name: "Rasipuram" },
        { name: "Ramnagar/Uttarakhand" },
        { name: "Jammalamadugu" },
        { name: "Nawanshahr" },
        { name: "Thoubal" },
        { name: "Athni" },
        { name: "Cherthala" },
        { name: "Sidhi" },
        { name: "Farooqnagar" },
        { name: "Peddapuram" },
        { name: "Chirkunda" },
        { name: "Pachora" },
        { name: "Madhepura" },
        { name: "Pithoragarh" },
        { name: "Tumsar" },
        { name: "Phalodi" },
        { name: "Tiruttani" },
        { name: "Rampura Phul" },
        { name: "Perinthalmanna" },
        { name: "Padrauna" },
        { name: "Pipariya" },
        { name: "Dalli-Rajhara" },
        { name: "Punganur" },
        { name: "Mattannur" },
        { name: "Mathura" },
        { name: "Thakurdwara" },
        { name: "Nandivaram-Guduvancheri" },
        { name: "Mulbagal" },
        { name: "Manjlegaon" },
        { name: "Wankaner" },
        { name: "Sillod" },
        { name: "Nidadavole" },
        { name: "Surapura" },
        { name: "Rajagangapur" },
        { name: "Sheikhpura" },
        { name: "Parlakhemundi" },
        { name: "Kalimpong" },
        { name: "Siruguppa" },
        { name: "Arvi" },
        { name: "Limbdi" },
        { name: "Barpeta" },
        { name: "Manglaur" },
        { name: "Repalle" },
        { name: "Mudhol" },
        { name: "Shujalpur" },
        { name: "Mandvi" },
        { name: "Thangadh" },
        { name: "Sironj" },
        { name: "Nandura" },
        { name: "Shoranur" },
        { name: "Nathdwara" },
        { name: "Periyakulam" },
        { name: "Sultanganj" },
        { name: "Medak" },
        { name: "Narayanpet" },
        { name: "Raxaul Bazar" },
        { name: "Rajauri" },
        { name: "Pernampattu" },
        { name: "Nainital" },
        { name: "Ramachandrapuram" },
        { name: "Vaijapur" },
        { name: "Nangal" },
        { name: "Sidlaghatta" },
        { name: "Punch" },
        { name: "Pandhurna" },
        { name: "Wadgaon Road" },
        { name: "Talcher" },
        { name: "Varkala" },
        { name: "Pilani" },
        { name: "Nowgong" },
        { name: "Naila Janjgir" },
        { name: "Mapusa" },
        { name: "Vellakoil" },
        { name: "Merta City" },
        { name: "Sivaganga" },
        { name: "Mandideep" },
        { name: "Sailu" },
        { name: "Vyara" },
        { name: "Kovvur" },
        { name: "Vadalur" },
        { name: "Nawabganj" },
        { name: "Padra" },
        { name: "Sainthia" },
        { name: "Siana" },
        { name: "Shahpur" },
        { name: "Sojat" },
        { name: "Noorpur" },
        { name: "Paravoor" },
        { name: "Murtijapur" },
        { name: "Ramnagar" },
        { name: "Sundargarh" },
        { name: "Taki" },
        { name: "Saundatti-Yellamma" },
        { name: "Pathanamthitta" },
        { name: "Wadi" },
        { name: "Rameshwaram" },
        { name: "Tasgaon" },
        { name: "Sikandra Rao" },
        { name: "Sihora" },
        { name: "Tiruvethipuram" },
        { name: "Tiruvuru" },
        { name: "Mehkar" },
        { name: "Peringathur" },
        { name: "Perambalur" },
        { name: "Manvi" },
        { name: "Zunheboto" },
        { name: "Mahnar Bazar" },
        { name: "Attingal" },
        { name: "Shahbad" },
        { name: "Puranpur" },
        { name: "Nelamangala" },
        { name: "Nakodar" },
        { name: "Lunawada" },
        { name: "Murshidabad" },
        { name: "Mahe" },
        { name: "Lanka" },
        { name: "Rudauli" },
        { name: "Tuensang" },
        { name: "Lakshmeshwar" },
        { name: "Zira" },
        { name: "Yawal" },
        { name: "Thana Bhawan" },
        { name: "Ramdurg" },
        { name: "Pulgaon" },
        { name: "Sadasivpet" },
        { name: "Nargund" },
        { name: "Neem-Ka-Thana" },
        { name: "Memari" },
        { name: "Nilanga" },
        { name: "Naharlagun" },
        { name: "Pakaur" },
        { name: "Wai" },
        { name: "Tarikere" },
        { name: "Malavalli" },
        { name: "Raisen" },
        { name: "Lahar" },
        { name: "Uravakonda" },
        { name: "Savanur" },
        { name: "Sirohi" },
        { name: "Udhampur" },
        { name: "Umarga" },
        { name: "Pratapgarh/Rajasthan" },
        { name: "Lingsugur" },
        { name: "Usilampatti" },
        { name: "Palia Kalan" },
        { name: "Wokha" },
        { name: "Rajpipla" },
        { name: "Vijayapura" },
        { name: "Rawatbhata" },
        { name: "Sangaria" },
        { name: "Paithan" },
        { name: "Rahuri" },
        { name: "Patti" },
        { name: "Zaidpur" },
        { name: "Lalsot" },
        { name: "Maihar" },
        { name: "Vedaranyam" },
        { name: "Nawapur" },
        { name: "Solan" },
        { name: "Vapi" },
        { name: "Sanawad" },
        { name: "Warisaliganj" },
        { name: "Revelganj" },
        { name: "Sabalgarh" },
        { name: "Tuljapur" },
        { name: "Simdega" },
        { name: "Musabani" },
        { name: "Kodungallur" },
        { name: "Phulabani" },
        { name: "Umreth" },
        { name: "Narsipatnam" },
        { name: "Nautanwa" },
        { name: "Rajgir" },
        { name: "Yellandu" },
        { name: "Sathyamangalam" },
        { name: "Pilibanga" },
        { name: "Morshi" },
        { name: "Pehowa" },
        { name: "Sonepur" },
        { name: "Pappinisseri" },
        { name: "Zamania" },
        { name: "Mihijam" },
        { name: "Purna" },
        { name: "Puliyankudi" },
        { name: "Shikarpur, Bulandshahr" },
        { name: "Umaria" },
        { name: "Porsa" },
        { name: "Naugawan Sadat" },
        { name: "Fatehpur Sikri" },
        { name: "Manuguru" },
        { name: "Udaipur" },
        { name: "Pipar City" },
        { name: "Pattamundai" },
        { name: "Nanjikottai" },
        { name: "Taranagar" },
        { name: "Yerraguntla" },
        { name: "Satana" },
        { name: "Sherghati" },
        { name: "Sankeshwara" },
        { name: "Madikeri" },
        { name: "Thuraiyur" },
        { name: "Sanand" },
        { name: "Rajula" },
        { name: "Kyathampalle" },
        { name: "Shahabad, Rampur" },
        { name: "Tilda Newra" },
        { name: "Narsinghgarh/Madhya Pradesh" },
        { name: "Chittur-Thathamangalam" },
        { name: "Malaj Khand" },
        { name: "Sarangpur" },
        { name: "Robertsganj" },
        { name: "Sirkali" },
        { name: "Radhanpur" },
        { name: "Tiruchendur" },
        { name: "Utraula" },
        { name: "Patratu" },
        { name: "Vijainagar, Ajmer" },
        { name: "Periyasemur" },
        { name: "Pathri" },
        { name: "Sadabad" },
        { name: "Talikota" },
        { name: "Sinnar" },
        { name: "Mungeli" },
        { name: "Sedam" },
        { name: "Shikaripur" },
        { name: "Sumerpur/Rajasthan" },
        { name: "Sattur" },
        { name: "Sugauli" },
        { name: "Lumding" },
        { name: "Vandavasi" },
        { name: "Titlagarh" },
        { name: "Uchgaon" },
        { name: "Mokokchung" },
        { name: "Paschim Punropara" },
        { name: "Sagwara" },
        { name: "Ramganj Mandi" },
        { name: "Tarakeswar" },
        { name: "Mahalingapura" },
        { name: "Dharmanagar" },
        { name: "Mahemdabad" },
        { name: "Manendragarh" },
        { name: "Uran" },
        { name: "Tharamangalam" },
        { name: "Tirukkoyilur" },
        { name: "Pen" },
        { name: "Makhdumpur" },
        { name: "Maner" },
        { name: "Oddanchatram" },
        { name: "Palladam" },
        { name: "Mundi" },
        { name: "Nabarangapur" },
        { name: "Mudalagi" },
        { name: "Samalkha" },
        { name: "Nepanagar" },
        { name: "Karjat" },
        { name: "Ranavav" },
        { name: "Pedana" },
        { name: "Pinjore" },
        { name: "Lakheri" },
        { name: "Pasan" },
        { name: "Puttur" },
        { name: "Vadakkuvalliyur" },
        { name: "Tirukalukundram" },
        { name: "Mahidpur" },
        { name: "Mussoorie" },
        { name: "Muvattupuzha" },
        { name: "Rasra" },
        { name: "Udaipurwati" },
        { name: "Manwath" },
        { name: "Adoor" },
        { name: "Uthamapalayam" },
        { name: "Partur" },
        { name: "Nahan" },
        { name: "Ladwa" },
        { name: "Mankachar" },
        { name: "Nongstoin" },
        { name: "Losal" },
        { name: "Sri Madhopur" },
        { name: "Ramngarh" },
        { name: "Mavelikkara" },
        { name: "Rawatsar" },
        { name: "Rajakhera" },
        { name: "Lar" },
        { name: "Lal Gopalganj Nindaura" },
        { name: "Muddebihal" },
        { name: "Sirsaganj" },
        { name: "Shahpura/Rajasthan" },
        { name: "Surandai" },
        { name: "Sangole" },
        { name: "Pavagada" },
        { name: "Tharad" },
        { name: "Mansa" },
        { name: "Umbergaon" },
        { name: "Mavoor" },
        { name: "Nalbari" },
        { name: "Talaja" },
        { name: "Malur" },
        { name: "Mangrulpir" },
        { name: "Soro" },
        { name: "Shahpura" },
        { name: "Vadnagar" },
        { name: "Raisinghnagar" },
        { name: "Sindhagi" },
        { name: "Sanduru" },
        { name: "Sohna" },
        { name: "Manavadar" },
        { name: "Pihani" },
        { name: "Safidon" },
        { name: "Risod" },
        { name: "Rosera" },
        { name: "Sankari" },
        { name: "Malpura" },
        { name: "Sonamukhi" },
        { name: "Shamsabad, Agra" },
        { name: "Nokha" },
        { name: "PandUrban Agglomeration" },
        { name: "Mainaguri" },
        { name: "Afzalpur" },
        { name: "Shirur" },
        { name: "Salaya" },
        { name: "Shenkottai" },
        { name: "Pratapgarh" },
        { name: "Vadipatti" },
        { name: "Nagarkurnool" },
        { name: "Savner" },
        { name: "Sasvad" },
        { name: "Rudrapur" },
        { name: "Soron" },
        { name: "Sholingur" },
        { name: "Pandharkaoda" },
        { name: "Perumbavoor" },
        { name: "Maddur" },
        { name: "Nadbai" },
        { name: "Talode" },
        { name: "Shrigonda" },
        { name: "Madhugiri" },
        { name: "Tekkalakote" },
        { name: "Seoni-Malwa" },
        { name: "Shirdi" },
        { name: "SUrban Agglomerationr" },
        { name: "Terdal" },
        { name: "Raver" },
        { name: "Tirupathur" },
        { name: "Taraori" },
        { name: "Mukhed" },
        { name: "Manachanallur" },
        { name: "Rehli" },
        { name: "Sanchore" },
        { name: "Rajura" },
        { name: "Piro" },
        { name: "Mudabidri" },
        { name: "Vadgaon Kasba" },
        { name: "Nagar" },
        { name: "Vijapur" },
        { name: "Viswanatham" },
        { name: "Polur" },
        { name: "Panagudi" },
        { name: "Manawar" },
        { name: "Tehri" },
        { name: "Samdhan" },
        { name: "Pardi" },
        { name: "Rahatgarh" },
        { name: "Panagar" },
        { name: "Uthiramerur" },
        { name: "Tirora" },
        { name: "Rangia" },
        { name: "Sahjanwa" },
        { name: "Wara Seoni" },
        { name: "Magadi" },
        { name: "Rajgarh (Alwar)" },
        { name: "Rafiganj" },
        { name: "Tarana" },
        { name: "Rampur Maniharan" },
        { name: "Sheoganj" },
        { name: "Raikot" },
        { name: "Pauri" },
        { name: "Sumerpur" },
        { name: "Navalgund" },
        { name: "Shahganj" },
        { name: "Marhaura" },
        { name: "Tulsipur" },
        { name: "Sadri" },
        { name: "Thiruthuraipoondi" },
        { name: "Shiggaon" },
        { name: "Pallapatti" },
        { name: "Mahendragarh" },
        { name: "Sausar" },
        { name: "Ponneri" },
        { name: "Mahad" },
        { name: "Lohardaga" },
        { name: "Tirwaganj" },
        { name: "Margherita" },
        { name: "Sundarnagar" },
        { name: "Rajgarh" },
        { name: "Mangaldoi" },
        { name: "Renigunta" },
        { name: "Longowal" },
        { name: "Ratia" },
        { name: "Lalgudi" },
        { name: "Shrirangapattana" },
        { name: "Niwari" },
        { name: "Natham" },
        { name: "Unnamalaikadai" },
        { name: "PurqUrban Agglomerationzi" },
        { name: "Shamsabad, Farrukhabad" },
        { name: "Mirganj" },
        { name: "Todaraisingh" },
        { name: "Warhapur" },
        { name: "Rajam" },
        { name: "Urmar Tanda" },
        { name: "Lonar" },
        { name: "Powayan" },
        { name: "P.N.Patti" },
        { name: "Palampur" },
        { name: "Srisailam Project (Right Flank Colony) Township" },
        { name: "Sindagi" },
        { name: "Sandi" },
        { name: "Vaikom" },
        { name: "Malda" },
        { name: "Tharangambadi" },
        { name: "Sakaleshapura" },
        { name: "Lalganj/Bihar" },
        { name: "Malkangiri" },
        { name: "Rapar" },
        { name: "Mauganj" },
        { name: "Todabhim" },
        { name: "Srinivaspur" },
        { name: "Murliganj" },
        { name: "Reengus" },
        { name: "Sawantwadi" },
        { name: "Tittakudi" },
        { name: "Lilong" },
        { name: "Rajaldesar" },
        { name: "Pathardi" },
        { name: "Achhnera" },
        { name: "Pacode" },
        { name: "Naraura" },
        { name: "Gorakhpur" },
        { name: "kushinagar" },
        { name: "Nakur" },
        { name: "Palai" },
        { name: "Morinda, India" },
        { name: "Manasa" },
        { name: "Nainpur" },
        { name: "Sahaspur" },
        { name: "Pauni" },
        { name: "Prithvipur" },
        { name: "Ramtek" },
        { name: "Silapathar" },
        { name: "Songadh" },
        { name: "Safipur" },
        { name: "Sohagpur" },
        { name: "Mul" },
        { name: "Sadulshahar" },
        { name: "Phillaur" },
        { name: "Sambhar" },
        { name: "Prantij" },
        { name: "Nagla" },
        { name: "Pattran" },
        { name: "Mount Abu" },
        { name: "Reoti" },
        { name: "Tenu dam-cum-Kathhara" },
        { name: "Panchla" },
        { name: "Sitarganj" },
        { name: "Pasighat" },
        { name: "Motipur" },
        { name: "O' Valley" },
        { name: "Raghunathpur" },
        { name: "Suriyampalayam" },
        { name: "Qadian" },
        { name: "Rairangpur" },
        { name: "Silvassa" },
        { name: "Nowrozabad (Khodargama)" },
        { name: "Mangrol" },
        { name: "Soyagaon" },
        { name: "Sujanpur" },
        { name: "Manihari" },
        { name: "Sikanderpur" },
        { name: "Mangalvedhe" },
        { name: "Phulera" },
        { name: "Ron" },
        { name: "Sholavandan" },
        { name: "Saidpur" },
        { name: "Shamgarh" },
        { name: "Thammampatti" },
        { name: "Maharajpur" },
        { name: "Multai" },
        { name: "Mukerian" },
        { name: "Sirsi" },
        { name: "Purwa" },
        { name: "Sheohar" },
        { name: "Namagiripettai" },
        { name: "Parasi" },
        { name: "Lathi" },
        { name: "Lalganj" },
        { name: "Narkhed" },
        { name: "Mathabhanga" },
        { name: "Shendurjana" },
        { name: "Peravurani" },
        { name: "Mariani" },
        { name: "Phulpur" },
        { name: "Rania" },
        { name: "Pali" },
        { name: "Pachore" },
        { name: "Parangipettai" },
        { name: "Pudupattinam" },
        { name: "Panniyannur" },
        { name: "Maharajganj" },
        { name: "Rau" },
        { name: "Monoharpur" },
        { name: "Mandawa" },
        { name: "Marigaon" },
        { name: "Pallikonda" },
        { name: "Pindwara" },
        { name: "Shishgarh" },
        { name: "Patur" },
        { name: "Mayang Imphal" },
        { name: "Mhowgaon" },
        { name: "Guruvayoor" },
        { name: "Mhaswad" },
        { name: "Sahawar" },
        { name: "Sivagiri" },
        { name: "Mundargi" },
        { name: "Punjaipugalur" },
        { name: "Kailasahar" },
        { name: "Samthar" },
        { name: "Sakti" },
        { name: "Sadalagi" },
        { name: "Silao" },
        { name: "Mandalgarh" },
        { name: "Loha" },
        { name: "Pukhrayan" },
        { name: "Padmanabhapuram" },
        { name: "Belonia" },
        { name: "Saiha" },
        { name: "Srirampore" },
        { name: "Talwara" },
        { name: "Puthuppally" },
        { name: "Khowai" },
        { name: "Vijaypur" },
        { name: "Takhatgarh" },
        { name: "Thirupuvanam" },
        { name: "Adra" },
        { name: "Piriyapatna" },
        { name: "Obra" },
        { name: "Adalaj" },
        { name: "Nandgaon" },
        { name: "Barh" },
        { name: "Chhapra" },
        { name: "Panamattom" },
        { name: "Niwai" },
        { name: "Bageshwar" },
        { name: "Tarbha" },
        { name: "Adyar" },
        { name: "Narsinghgarh" },
        { name: "Warud" },
        { name: "Asarganj" },
        { name: "Sarsod" }
    ],

    fillDropdown: function (listName, ddl, type) {
        var list = dropdown[listName];
        if (type != null) {
            list = list.filter(function (el) {
                return el.type == type;
            });
        }

        $(ddl).html('');
        $(ddl).append('<option value="">Select</option>');
        $.each(list, function (i, v) {
            $(ddl).append('<option value="' + v.name + '">' + v.name + '</option>');
        });
    }


}